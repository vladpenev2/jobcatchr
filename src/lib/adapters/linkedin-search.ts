...
function extractLinkedInSlug(url: string | null | undefined): string | null {
  if (!url) return null
  // Match /company/slug or /company/slug/
  const match = url.match(/\/company\/([^/?#]+)/)
  return match ? match[1] : null
}

export class LinkedInSearchAdapter extends BaseAdapter {
  ...
  normalize(rawItem: unknown): UnifiedJob {
    ...
    return {
      ...
      organization_linkedin_slug:
        extractLinkedInSlug(item.linkedin_org_url as string) ??
        (item.linkedin_org_slug as string) ??
        null,
      url: item.url as string,
      description_text: (item.description_text as string) ?? null,
      description_html: (item.description_html as string) ?? null,
      location_raw: (item.locations_raw as Record<string, unknown>) ?? null,
      ...
      salary_raw: (item.salary_raw as Record<string, unknown>) ?? null,
      date_posted: (item.date_posted as string) ?? null,
